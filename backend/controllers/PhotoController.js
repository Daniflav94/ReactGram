...
const commentPhoto = async (req, res) => {
  ...
  const user = await User.findById(reqUser._id); //precisamos de mais info do user, como nome e imagem
...
const searchPhotos = async(req, res) => {
  const {q} = req.query // não vem como parâmetro e sim como query

  const photos = await Photo.find({title: new RegExp(q, "i")}).exec() 
  //expressão regular, vai buscar qualquer título que contenha a string passada pelo q
